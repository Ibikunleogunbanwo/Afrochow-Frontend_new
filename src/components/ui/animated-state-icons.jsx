...
/**
 * All icons accept:
 *   size?     – px size of the SVG (default 40)
 *   color?    – stroke/fill colour (default "currentColor")
 *   className?
 *   state?    – CONTROLLED boolean; when provided the icon reflects this
 *               value and auto-toggle is disabled.
 *   duration? – auto-toggle interval in ms (only used when `state` is undefined)
 */

/** Internal hook: uses controlled `state` if given, otherwise auto-toggles */
function useIconState(state, duration = 2000) {
  const [auto, setAuto] = useState(false);
  useEffect(() => {
    if (state !== undefined) return;
    const id = setInterval(() => setAuto(v => !v), duration);
    return () => clearInterval(id);
  }, [state, duration]);
  return state !== undefined ? state : auto;
}

/* ─── 1. LOADING → SUCCESS ─── spinner morphs into checkmark ─────────── */
export function SuccessIcon({ size = 40, color = "currentColor", className, state, duration = 2200 }) {
  const done = useIconState(state, duration);
  return (
...
export function MenuCloseIcon({ size = 40, color = "currentColor", className, state, duration = 2000 }) {
...
/* ─── 3. PLAY → PAUSE ─────────────────────────────────────────────────── */
export function PlayPauseIcon({ size = 40, color = "currentColor", className, state, duration = 2400 }) {
  const playing = useIconState(state, duration);
  return (
    <svg viewBox="0 0 40 40" fill="none" className={cn("", className)} style={{ width: size, height: size }}>
...
/* ─── 4. LOCK → UNLOCK ─────────────────────────────────────────────────── */
export function LockUnlockIcon({ size = 40, color = "currentColor", className, state, duration = 2600 }) {
  const unlocked = useIconState(state, duration);
  return (
    <svg viewBox="0 0 40 40" fill="none" className={cn("", className)} style={{ width: size, height: size }}>
...
/* ─── 5. COPY → COPIED ─────────────────────────────────────────────────── */
export function CopiedIcon({ size = 40, color = "currentColor", className, state, duration = 2200 }) {
  const copied = useIconState(state, duration);
  return (
    <svg viewBox="0 0 40 40" fill="none" className={cn("", className)} style={{ width: size, height: size }}>
...
export function NotificationIcon({ size = 40, color = "currentColor", className, state, duration = 2800 }) {
  const notif = useIconState(state, duration);
  return (
...
/* ─── 7. HEART → FILLED ─────────────────────────────────────────────────── */
export function HeartIcon({ size = 40, color = "currentColor", className, state, duration = 2000 }) {
  const filled = useIconState(state, duration);
  return (
    <svg viewBox="0 0 40 40" fill="none" className={cn("", className)} style={{ width: size, height: size }}>
      <motion.path
...
/* ─── 8. DOWNLOAD → DONE ─────────────────────────────────────────────────── */
export function DownloadDoneIcon({ size = 40, color = "currentColor", className, state, duration = 2400 }) {
  const done = useIconState(state, duration);
  return (
    <svg viewBox="0 0 40 40" fill="none" className={cn("", className)} style={{ width: size, height: size }}>
      ...
      <AnimatePresence mode="wait">
        {done ? (
          ...
          <motion.g key="arrow" initial={{ y: -4, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: 8, opacity: 0 }}
...
export function SendIcon({ size = 40, color = "currentColor", className, state, duration = 2600 }) {
  const sent = useIconState(state, duration);
  return (
    <svg viewBox="0 0 40 40" fill="none" className={cn("", className)} style={{ width: size, height: size }}>
      <motion.g
...
export function ToggleIcon({ size = 40, color = "currentColor", className, state, duration = 1800 }) {
  const on = useIconState(state, duration);
  return (
    <svg viewBox="0 0 40 40" fill="none" className={cn("", className)} style={{ width: size, height: size }}>
...
/* ─── 11. EYE → HIDDEN ──────────────────────────────────────────────────── */
export function EyeToggleIcon({ size = 40, color = "currentColor", className, state, duration = 2200 }) {
  const hidden = useIconState(state, duration);
  return (
    <svg viewBox="0 0 40 40" fill="none" className={cn("", className)} style={{ width: size, height: size }}>
...
export function VolumeIcon({ size = 40, color = "currentColor", className, state, duration = 2400 }) {
  const muted = useIconState(state, duration);
  return (
    <svg viewBox="0 0 40 40" fill="none" className={cn("", className)} style={{ width: size, height: size }}>
