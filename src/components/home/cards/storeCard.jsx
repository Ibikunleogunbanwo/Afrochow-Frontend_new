...
const StoreCard = ({ store, isLoading = false, priority = false }) => {
    ...
    const imageUrl = popularItems?.length > 0 ? popularItems[0].imageUrl : '/image/amala.jpg';
    ...
    const handleCardClick = (e) => {
        ...
        if (!isAuthenticated) {
            ...
            // Use vendorPublicId if available, otherwise fall back to storeId
        ...
        >
            <div className="relative overflow-hidden transition-all duration-300 bg-white rounded-2xl shadow-md hover:shadow-2xl hover:-translate-y-1">
                ...
                <div className="p-5 space-y-3">
                    ...
                    {/* Delivery Fee & Location (Optional) */}
                    {(deliveryFee !== undefined || location) && (
                        <div className="flex items-center justify-between pt-2 text-xs text-gray-500">
                            {deliveryFee !== undefined && (
                                <span className="font-semibold text-orange-600">
                  {deliveryFee === 0 || deliveryFee === '0' ? '🎉 Free Delivery' : `$${deliveryFee} delivery`}
                </span>
                            )}
                            {location && (
